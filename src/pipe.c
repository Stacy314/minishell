...
/*   Updated: 2025/02/22 18:15:49 by apechkov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *find_command_path(const char *cmd, char **env)
{
    ...
    token = strtok(paths, ":"); //need to change
    while (token)
    {
...
void execute_pipeline(t_cmd **cmd, t_data *data, char **env)
{
    ...
    {
		int status_pid;
        status_pid = waitpid(pids[i], &status, 0);
        if (/* status_pid == last command && */WIFEXITED(status))
