...
void	execute_for_one(t_token **tokens, t_cmd *cmd, t_data *data, char **env)
{
	...
	i = 0;
	{
		...
		{
			if (ft_strncmp(tokens[i]->value, "echo", ft_strlen(tokens[i]->value)) == 0)
				return ((void)builtin_echo(cmd, data));
			else if (ft_strncmp(tokens[i]->value, "cd", ft_strlen(tokens[i]->value)) == 0)
				return ((void)builtin_cd(cmd, data));
			else if (ft_strncmp(tokens[i]->value, "pwd", ft_strlen(tokens[i]->value)) == 0)
				return ((void)builtin_pwd(cmd, data));
			else if (ft_strncmp(tokens[i]->value, "export", ft_strlen(tokens[i]->value)) == 0)
				return ((void)builtin_export(cmd, data));
			else if (ft_strncmp(tokens[i]->value, "unset", ft_strlen(tokens[i]->value)) == 0)
				return ((void)builtin_unset(cmd, data));
			else if (ft_strncmp(tokens[i]->value, "env", ft_strlen(tokens[i]->value)) == 0)
				return ((void)builtin_env(data));
			else if (ft_strncmp(tokens[i]->value, "exit", ft_strlen(tokens[i]->value)) == 0)
